#include <iostream>
unsigned int djbhashh(std::string ulaz, unsigned int max) {
...
unsigned int mojhashh(int ulaz, unsigned int max) {
...
public:
    ...
    Tip2& operator [](Tip1 kljuc)
    {
        ...
        return hash_dodaj(kljuc);

}

Tip2& hash_dodaj(Tip1 kljuc) {
        unsigned int j = 0;
 ...
 int niz[10000];
   ...
   poc = clock();
    ...
    izvrsenje = (kraj-poc)/(CLOCKS_PER_SEC/1000000);
     ...
     poc = clock();
    mapa.obrisi(niz[5000]);
    kraj = clock();
    izvrsenje = (kraj-poc)/(CLOCKS_PER_SEC/1000000);
     std::cout << "NizMapa: Element se brisao " << izvrsenje << " mikrosekundi." << std::endl;
    ...
    izvrsenje = (kraj-poc)/(CLOCKS_PER_SEC/1000000);
     std::cout << "Binarno stablo: Pretraga se izvrsavala " << izvrsenje << " mikrosekundi." << std::endl;
      poc = clock();
    binmapa[m];
  // binmapa.traziKljuc(m);
    kraj = clock();
    izvrsenje = (kraj-poc)/(CLOCKS_PER_SEC/1000000);
     std::cout << "Binarno stablo: Element se dodavao " << izvrsenje << " mikrosekundi." << std::endl;
     poc = clock();
    binmapa.obrisi(niz[5000]);
    kraj = clock();
    izvrsenje = (kraj-poc)/(CLOCKS_PER_SEC/1000000);
     std::cout << "Binarno stablo: Element se brisao " << izvrsenje << " mikrosekundi." << std::endl;
    HashMapa<int, int> hashmapa;
    hashmapa.definisiHashFunkciju(mojhashh);
    poc = clock();
     for(int i=0; i<10000; i++)
    hashmapa[niz[i]] = i;
    kraj = clock();
    izvrsenje = (kraj-poc)/(CLOCKS_PER_SEC/1000);
   // std::cout << "HashMapa se formirala " << izvrsenje << " ms." << std::endl;
    poc = clock();
    auto t = hashmapa.hash_trazi(niz[k]);
    delete t;
    kraj = clock();
    izvrsenje = (kraj-poc)/(CLOCKS_PER_SEC/1000000);
    std::cout << "HashMapa: Pretraga se izvrsavala " << izvrsenje << " mikrosekundi." << std::endl;
    poc = clock();
    hashmapa.hash_dodaj(m);
    kraj = clock();
    izvrsenje = (kraj-poc)/(CLOCKS_PER_SEC/1000000);
     std::cout << "HashMapa: Element se dodavao " << izvrsenje << " mikrosekundi." << std::endl;
     poc = clock();
    hashmapa.obrisi(niz[5000]);
    kraj = clock();
    izvrsenje = (kraj-poc)/(CLOCKS_PER_SEC/1000000);
     std::cout << "HashMapa: Element se brisao " << izvrsenje << " mikrosekundi." << std::endl;
     //Nakon testiranja zakljucila sam da najbolje preformanse ima mapa implementirana pomocu binarnog stabla
     //nakon nje slijedi mapa implementirana pomocu niza, a zatim hash mapa,
     // u principu bi trebalo da hash mapa ima kompleksnost O(1) i da u usporedbi sa niz mapom i binarnom mapom
     // bude najbrza, dodatnim testiranjem sam utvrdila da to nije tako iz razloga sto se javilo mnogo kolizija
     //vec pri samom formiranju mape od 10000 elemenata, tako da trazenje na prvoj predodredjenoj poziciji
     //nije bilo uspjesno, pa je automatski narusena kompleksnost O(1)
     //razlog za to je trivijalan nacin rjesavanja kolizije otvorenim adresiranjem 
     //binarna mapa ima uglavnom isto vrijeme izvrsavanja operacija zato jer su u mapi random brojevi
     // kod niz mape zavisi koji je po redu element dodan,okvirnokompleksonst je O(n)
    return 0; 
